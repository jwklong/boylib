...
int Font::getStringWidth(const BoyLib::UString &str)
{
	int width = 0;
...
float Font::drawString(Graphics *g, const BoyLib::UString &str, float scale)
{
	scale *= mScale;

	BoyLib::Transform old1 = g->getTransform().clone();
    g->getTransform().preScale(scale);
	...
	for (int i=0 ; i<numChars ; i++)
	{
		...
		float dx = kerning + fcCurr->width/2.0f + prevCharWidth/2.0f;
		g->getTransform().translate(dx, 0);
		...
		prevCharWidth = (fcCurr->width + mSpacing);

		// the current char is the new previous char:
		fcPrev = fcCurr;
	}
	g->getTransform().set(old1);
...
void Font::loadCharList(BoyLib::UStringStream &fontStream, std::vector<wchar_t> &charList)
{
	BoyLib::UString buf;
...
void Font::loadWidthList(BoyLib::UStringStream &fontStream, std::vector<int> &widthList)
{
	BoyLib::UString buf;
...
void Font::loadRectList(BoyLib::UStringStream &fontStream, std::vector<BoyLib::Rect> &rectList)
{
	BoyLib::UString buf;
...
void Font::loadOffsetList(BoyLib::UStringStream &fontStream, std::vector<BoyLib::Vector2> &offsetList)
{
	BoyLib::UString buf;
	int x,y;
...
void Font::loadKerningPairs(BoyLib::UStringStream &fontStream, std::vector<BoyLib::UString> &kerningPairs)
{
	BoyLib::UString buf;
...
void Font::loadKerningValues(BoyLib::UStringStream &fontStream, std::vector<int> &kerningValues)
{
	BoyLib::UString buf;
...
bool Font::init()
{
	...
	BoyLib::UStringStream fontStream( pFileData );
	delete [] pFileData;
	...
	std::vector<BoyLib::UString> kerningPairs;
	...
	{
		BoyLib::UString pair = kerningPairs[i];
