...
{
	if (!isValidMouseCoord(x,y) || (mPosition.x == x && mPosition.y == y))
	{
		return;
	}
...
void Mouse::fireDownEvent(Button button)
{
	...
	if (mListeners.size()>0 && isEnabled() && mButtonsEnabled)
	{
		...
		for (int i=0 ; i<numListeners ; i++)
		{
			mListeners[i]->mouseButtonDown(this,button);
		}
	}
	...
	for (int i=0 ; i<numListeners ; i++)
	{
		mLowLevelListeners[i]->mouseButtonDown(this,button);
	}
}
