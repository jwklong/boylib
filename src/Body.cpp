...
Body::Body(BodyType type, float posx, float posy, float velx, float vely)
{
	...
	case BIG_ASTEROID:
		mImage = Boy::Environment::getImage(
			BoyLib::UString::format("IMAGE_ASTEROID_BIG_%d",randi(1,3)).toUtf8());
		mRadius = 100;
		do
		{
	...
	case MEDIUM_ASTEROID:
		mImage = Boy::Environment::getImage(
			BoyLib::UString::format("IMAGE_ASTEROID_MEDIUM_%d",randi(1,3)).toUtf8());
		mRadius = 50;
	...
	case SMALL_ASTEROID:
		mImage = Boy::Environment::getImage(
			BoyLib::UString::format("IMAGE_ASTEROID_SMALL_%d",randi(1,3)).toUtf8());
		mRadius = 25;
